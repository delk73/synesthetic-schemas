...
Also round-trips JSON -> model -> JSON (exclude None) and diffs shapes.
...
def validate_file(p: pathlib.Path) -> list[str]:
    ...
    try:
        ...
        if _normalize(data) != _normalize(out):
            errs.append(f"{p.name}: round-trip mismatch (shape changed)")
    except Exception as e:
...
def main() -> int:
    ...
    files = sorted([*EXAMPLES_DIR.glob("*.json"), *EXAMPLES_DIR.glob("**/*.json")])
    if not files:
